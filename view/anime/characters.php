    ...
    <div class="grow flex flex-col w-full md:w-auto justify-between leading-normal self-start">
      ...
      <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-6">
          <?php foreach ($charactersAndStaff->getCharacters() as $indexChar => $char) : ?>
              ...
              <div class="flex self-end -space-x-4" style="margin-inline-start: auto">
                <?php foreach ($char->getVoiceActors() as $key => $item) : ?>
                  <?php if ($item->getLanguage() == 'Japanese') : ?>
                    <div>
                      <div id="tooltip-#<?= $indexChar ?>" role="tooltip" class="absolute z-10 invisible inline-block opacity-0 tooltip transition-opacity bg-gray-100 text-gray-800 text-xs font-medium mr-1 px-2 py-1 rounded border border-gray-700 dark:bg-[#181818] shadow-lg dark:text-gray-300 duration-150">
                        <?= $item->getPerson()->getName() . ' (' . $item->getLanguage() . ')' ?>
                        <div class="tooltip-arrow" data-popper-arrow></div>
                      </div>
                      <img data-tooltip-target="tooltip-#<?= $indexChar ?>" class="cursor-pointer w-10 h-10 border-2 border-white object-cover rounded-full dark:border-gray-800" src="<?= $item->getPerson()->getImages()->getJpg()->getImageUrl() ?>" alt="">
                    </div>
                  <?php endif; ?>
                <?php endforeach; ?>
              </div>
      ...
      <div class="mt-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-6">
          <?php foreach ($charactersAndStaff->getStaff() as $indexStaff => $staff) : ?>
            <div class="max-w-full flex flex-col md:flex-row items-center space-x-3 p-3 bg-[#181818] dark:hover:bg-[#202020] group text-white rounded">
              ...
              <div class="flex flex-col text-center md:text-left">
                ...
                <div class="flex mt-2 flex-wrap">
                  <?php foreach ($staff->getPositions() as $index => $position) : ?>
                    <?= ($index < $more) ? "<span class='bg-gray-100 text-gray-800 text-xs font-medium mr-2 px-2 py-0.5 rounded border border-gray-700 dark:bg-[#202020] dark:group-hover:bg-[#181818] dark:text-gray-300'>$position</span>" : ''; ?>
                  <?php endforeach; ?>
                  <?php
